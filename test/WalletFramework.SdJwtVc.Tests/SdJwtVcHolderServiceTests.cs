...
public class SdJwtVcHolderServiceTests
{
    ...
    public SdJwtVcHolderServiceTests()
    {
        ...
        var walletRecordServiceMock = new Mock<IWalletRecordService>().Object;
        _service = new SdJwtVcHolderService(holder, sdJwtSignerMock, walletRecordServiceMock);
    }
    ...
    public async Task Can_Create_Presentation_For_Example_4A()
    {
        ...
        var sdJwtRecord = new SdJwtRecord(issuedSdJwt, new Dictionary<string, ClaimMetadata>(), new List<SdJwtDisplay>(), keyId, CredentialSetId.CreateCredentialSetId());
        var claimsToDisclose = new[] { "given_name", "address.street_address", "nationalities[0]" };

        // Act
        var result = await _service.CreatePresentation(sdJwtRecord, claimsToDisclose, Option<IEnumerable<string>>.None);
