...
using Xunit;
using static WalletFramework.Mdoc.Mdoc;
...
public class MdocTests
{
    ...
    public void Can_Encode()
    {
        ...
        ValidMdoc(encodedSample).Match(
            mdoc =>
            {
                var encoded = mdoc.Encode();
                ValidMdoc(encoded).Match(
                    sut =>
                    {
                        // TODO: Assertion
                        // var equals = sut.Equals(mdoc);
                        // if (!equals) 
                        //     Assert.Fail("encoded mdoc is not equals to input mdoc");
                    },
                    _ => Assert.Fail("Encoded mdoc could not be decoded again")
                );
            },
    ...
    public void Can_Parse_Mdoc()
    {
        ...
        ValidMdoc(encoded).Match(sut =>
            {
                // Assert
                sut.DocType.Value.Should().Be(Samples.DocType);

                var issuerSignedItems = sut.IssuerSigned.NameSpaces[Samples.DocTypeNameSpace];
                ...
                issuerAuth.Payload.DocType.Value.Should().Be(Samples.DocType);
    ...
    public void Can_Selectively_Disclose()
    {
        ...
        ValidMdoc(sample).Match(
            sut =>
            {
                ...
                // Act
                sut.SelectivelyDisclose(Samples.DocTypeNameSpace, disclosures);

                // Assert
                var items = sut.IssuerSigned.NameSpaces.Value[Samples.DocTypeNameSpace];
    ...
    public void Can_Selectively_Disclose_And_Encode()
    {
        ...
        ValidMdoc(sample).Match(
            mdoc =>
            {
                ...
                // Act
                mdoc.SelectivelyDisclose(Samples.DocTypeNameSpace, disclosures);
                var encoded = mdoc.Encode();
                ...
                ValidMdoc(encoded).Match(
                    ...
                    {
                        // TODO: Assertion
                        // var equals = mdoc.Equals(sampleMdoc);
                        // if (!equals) 
                        //     Assert.Fail("encoded mdoc is not equals to input mdoc");
                    },
                    _ => Assert.Fail("Encoded mdoc could not be decoded again")
    ...
    public void Mdoc_With_Invalid_Digests_Is_Rejected()
    {
        ...
        var msoBytes = cbor[IssuerSignedLabel][IssuerAuthLabel][2].GetByteString();
        ...
        cbor[IssuerSignedLabel][IssuerAuthLabel][2] = encodedMso;
        ...
        ValidMdoc(corruptedMdoc).Match(
            // Assert
            _ => Assert.Fail("mdoc must not be valid"),
            sut =>
            {
