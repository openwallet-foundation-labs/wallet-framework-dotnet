        ...
        var result = presentationRequest.CandidateQueryResult.Candidates.Match(
            candidateSets =>
            {
                ...
                var newSets = candidateSets.Select((set, setIdx) =>
                    set with
                    {
                        Candidates = [.. set.Candidates.Select((c, candIdx) => updatedCandidates[(setIdx, candIdx)])]
                    }).ToList();
        ...
        var result = presentationRequest.CandidateQueryResult.Candidates.Match(
            candidateSets =>
            {
                ...
                foreach (var txData in transactionDatas)
                {
                    ...
                    {
                        return new[] { tuple.candidate }.FindCandidateForTransactionData(txData).IsSome;
                    });
                    ...
                    var updated = found.candidate.AddTransactionData(txData);
                    updatedCandidates[(found.setIdx, found.candIdx)] = updated;
                }
