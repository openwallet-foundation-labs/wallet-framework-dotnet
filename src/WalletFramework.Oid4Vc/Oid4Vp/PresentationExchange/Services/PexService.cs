...
{
    public PexService(IMdocStorage mdocStorage)
    {
    ...
    /// <inheritdoc />
    public Task<PresentationSubmission> CreatePresentationSubmission(PresentationDefinition presentationDefinition,
    ...
    /// <inheritdoc />
    public virtual async Task<CredentialCandidates[]> FindCredentialCandidates(
        SdJwtRecord[] credentials,
        InputDescriptor[] inputDescriptors)
    {
        ...
        foreach (var inputDescriptor in inputDescriptors)
        {
            ...
            var matchingCredentials = await _filterMatchingCredentialsForFields(
                credentials,
    ...
    private async Task<List<ICredential>> _filterMatchingCredentialsForFields(
        SdJwtRecord[] records,
        Field[] fields)
    {
        var sdJwtRecords = new List<SdJwtRecord>();
        foreach (var record in records)
        {
            ...
            if (isAMatch)
                sdJwtRecords.Add(record);
        }

        var a = sdJwtRecords.Cast<ICredential>();
        
        var mdocRecords = await _mdocStorage.List();
        // TODO: Implement mdoc PEX
        mdocRecords.Match(
            mdocs =>
            {
                // foreach (var mdocRecord in mdocs)
                // {
                //     var x = fields.All(field =>
                //     {
                //
                //     });
                // }

                var b = mdocs.Cast<ICredential>();
                a = a.Concat(b);
            },
            () => { }
        );
